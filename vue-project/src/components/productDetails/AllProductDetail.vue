...
import { allProduct } from '../../data/pants.js';
...
const findProductById = (id) => allProduct.find(product => product.id === id);
...
onMounted(async () => {
    try {
        ...
        if (product.value) {
        selectedSize.value = Object.keys(product.value.size)[0];
        selectedColor.value = Object.keys(product.value.color)[0];
        }
    } catch (e) {
