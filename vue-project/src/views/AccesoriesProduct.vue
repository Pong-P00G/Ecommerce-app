...
onMounted(async () => {
  try {
    ...
    productCard.value = Accesorie.map(Accesorie => ({
      ...Accesorie,
      image: Accesorie.image, 
      price: parseFloat(Accesorie.price.replace('$', '')), 
      originalPrice: parseFloat(Accesorie.price.replace('$', '')) * 1.3,
...
const addToCart = async (Accesorie) => {
    try {
    const qty = Math.max(1, quantity.value[Accesorie.id] || 1);
    const cartItem = {
        id: Accesorie.id,
        name: Accesorie.name,
        size: selectSize.value[Accesorie.id],
        color: selectColor.value[Accesorie.id],
        quantity: qty,
        price: Accesorie.price,
        originalPrice: Accesorie.originalPrice,
        description: Accesorie.description,
        image: Accesorie.image,
        category: Accesorie.category,
        addedAt: new Date().toISOString()
    };
    const existingItems = JSON.parse(localStorage.getItem('cartItems')) || [];
    const existingIndex = existingItems.findIndex((item) =>
        item.id === Accesorie.id &&
    ...
    if (existingIndex !== -1) {
        ...
        showToast(`${Accesorie.name} added to cart! 🛒`, 'success');
    }
...
const quickAddToCart = (Accesorie) => {
    addToCart(Accesorie);
};

const buyNow = (Accesorie) => {
    addToCart(Accesorie);
    ...
    <div>
        ...
        <section class="bg-gradient-to-br from-gray-800 via-blue-900 to-purple-900 text-white">
            ...
            <div class="text-center">
                ...
                Premium T-Accesorie Collection
                </h1>
                <p class="text-lg sm:text-xl lg:text-2xl text-blue-100 max-w-3xl mx-auto mb-8">
                Discover our curated selection of high-quality, comfortable t-Accesories designed for the modern lifestyle.
                </p>
        ...
        <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8 py-6 sm:py-8 lg:py-12">
            ...
            <div v-for="Accesorie in filteredProducts" :key="Accesorie.id" class="bg-white rounded-xl sm:rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group hover:translate-y-2">
                <!-- Image Container -->
                <div class="relative aspect-square overflow-hidden bg-gray-100">
                <!-- Badges -->
                <div class="absolute top-2 sm:top-4 left-2 sm:left-4 z-10 space-y-2">
                    <span v-if="Accesorie.isNew" class="inline-block px-2 py-1 bg-green-500 text-white text-xs font-bold rounded-full">
                    NEW
                    </span>
                    <span v-if="Accesorie.isBestseller" class="inline-block px-2 py-1 bg-orange-500 text-white text-xs font-bold rounded-full">
                    <TrendingUp class="w-3 h-3 inline mr-1" />
                    BESTSELLER
                    </span>
                </div>
                <!-- Wishlist Button -->
                <button @click="toggleWishlist(Accesorie.id)" class="absolute top-2 sm:top-4 right-2 sm:right-4 z-10 p-1.5 sm:p-2 bg-white/90 backdrop-blur-sm rounded-full shadow-sm hover:bg-white transition-all duration-200">
                    <Heart :class="wishlist.has(Accesorie.id) ? 'text-red-500 fill-current' : 'text-gray-600 hover:text-red-500'" class="w-4 h-4 sm:w-5 sm:h-5" />
                </button>
                <!-- Stock indicator -->
                <div v-if="Accesorie.stock < 5" class="absolute top-2 right-16 sm:right-20 z-10">
                    <span class="inline-block px-2 py-1 bg-red-500 text-white text-xs font-bold rounded-full">
                    Only {{ Accesorie.stock }} left!
                    </span>
                </div>
                <!-- Product Image -->
                <img :src="Accesorie.image" :alt="Accesorie.name" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                <!-- Quick Actions Overlay -->
                <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-300 flex items-center justify-center">
                    <div class="opacity-0 group-hover:opacity-100 flex space-x-2">
                    <router-link :to="`/Accesorie/${Accesorie.id}`" class="px-3 sm:px-4 py-1.5 sm:py-2 bg-white text-gray-900 rounded-lg font-medium hover:bg-gray-100 transition-all duration-200 text-xs sm:text-sm">
                        <span class="hidden sm:inline">Quick View</span>
                        <Eye class="w-4 h-4 sm:hidden" />
                    </router-link>
                    <button @click="quickAddToCart(Accesorie)" class="px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-all duration-200 text-xs sm:text-sm">
                ...
                <div class="space-y-1.5 sm:space-y-2">
                    ...
                    <button v-for="(colorName, colorCode) in Accesorie.color" :key="colorCode" @click="updateSelectedColor(Accesorie.id, colorCode)" class="relative group/color">
                        <div class="w-6 h-6 sm:w-8 sm:h-8 rounded-full border-2 transition-all duration-200" :style="{ backgroundColor: getColorHex(colorName) }"
                            :class="selectColor[Accesorie.id] === colorCode? 'border-blue-500 ring-1 sm:ring-2 ring-blue-200 scale-110' : 'border-gray-300 hover:border-gray-400 hover:scale-105'">
                        <div v-if="selectColor[Accesorie.id] === colorCode" class="absolute inset-0 flex items-center justify-center">
                ...
                <div class="space-y-1.5 sm:space-y-2">
                    ...
                    <button v-for="(sizeName, sizeCode) in Accesorie.size" :key="sizeCode" @click="updateSelectedSize(Accesorie.id, sizeCode)"
                        ...
                        :class="selectSize[Accesorie.id] === sizeCode ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-white text-gray-700 border-gray-300 hover:border-blue-400'">
                        {{ sizeName }}
                    </button>
                    </div>
                </div>
                ...
                <div class="space-y-1.5 sm:space-y-2">
                    ...
                    <div class="flex items-center border border-gray-300 rounded-md sm:rounded-lg overflow-hidden">
                        <button @click="decrementQuantity(Accesorie.id)" :disabled="quantity[Accesorie.id] <= 1" class="p-1.5 sm:p-2 hover:bg-gray-100 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                        ...
                        <span class="text-sm sm:text-base font-semibold text-gray-900">{{ quantity[Accesorie.id] || 1 }}</span>
                        </div>
                        <button @click="incrementQuantity(Accesorie.id)" :disabled="quantity[Accesorie.id] >= Math.min(10, Accesorie.stock)" class="p-1.5 sm:p-2 hover:bg-gray-100 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 pt-2">
                    <button @click="addToCart(Accesorie)"
                    ...
                    <button @click="buyNow(Accesorie)" class="sm:flex-shrink-0 px-3 sm:px-4 py-2.5 sm:py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all duration-200 flex items-center justify-center font-medium text-sm sm:text-base hover:scale-[1.02]">
                    <Zap class="w-4 h-4 sm:w-5 sm:h-5 mr-2 sm:mr-0" />
                    <span class="sm:hidden">Buy Now</span>
                    </button>
                    <router-link :to="`/Accesorie/${Accesorie.id}`" class="sm:flex-shrink-0 px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors duration-200 flex items-center justify-center">
                ...
                <!-- Stock Status -->
                <div class="text-center">
                    <span v-if="Accesorie.stock > 10" class="text-xs text-green-600 font-medium">✓ In Stock</span>
                    <span v-else-if="Accesorie.stock > 0" class="text-xs text-orange-600 font-medium">⚠ Limited Stock ({{ Accesorie.stock }} left)</span>
            ...
            <div v-for="Accesorie in filteredProducts" :key="Accesorie.id" class="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 p-4 sm:p-6">
                <div class="flex flex-col sm:flex-row gap-6">
                <!-- Image -->
                <div class="w-full sm:w-48 h-48 sm:h-32 relative overflow-hidden rounded-lg bg-gray-100 flex-shrink-0">
                    <img :src="Accesorie.image" :alt="Accesorie.name" class="w-full h-full object-cover" loading="lazy" />
                    <button @click="toggleWishlist(Accesorie.id)" class="absolute top-2 right-2 p-1.5 bg-white/90 backdrop-blur-sm rounded-full shadow-sm">
                    <Heart :class="wishlist.has(Accesorie.id) ? 'text-red-500 fill-current' : 'text-gray-600'" class="w-4 h-4" />
                    </button>
                </div>
                <!-- Details -->
                <div class="flex-1 space-y-3">
                    <div class="flex justify-between items-start">
                    <h3 class="text-lg font-semibold text-gray-900">{{ Accesorie.name }}</h3>
                    <div class="flex items-center space-x-1 text-sm text-gray-500">
                        <Star class="w-4 h-4 text-yellow-400 fill-current" />
                        <span>{{ Accesorie.rating }}</span>
                    </div>
                    </div>
                    <p class="text-gray-600 text-sm">{{ Accesorie.description }}</p>
                    <div class="flex items-center space-x-4">
                    <span class="text-xl font-bold text-gray-900">${{ Accesorie.price.toFixed(2) }}</span>
                    <span class="text-gray-400 line-through">${{ Accesorie.originalPrice.toFixed(2) }}</span>
                    ...
                    <router-link :to="`/Accesorie/${Accesorie.id}`" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                        View Details →
                    </router-link>
                    </div>
                </div>
